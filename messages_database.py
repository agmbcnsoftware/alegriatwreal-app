...
def get_all_messages():
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("""
        SELECT m.whatsapp_number,m.whatsapp_profile, m.message, m.timestamp, m.sender
        FROM messages m
...
def get_all_reservations():
    with get_connection() as conn:
        ...
        # Obtener reservas del día actual sin recordatorio enviado
        cursor.execute("""
        SELECT id, user_name, user_surname, whatsapp_number, class_type, class_weekday_hour, class_date, class_time, reminder_sent, created_at
...
def get_tomorrow_reservations():
    with get_connection() as conn:
        ...
        return cursor     
def get_reservations_from_user(whatsapp_number):
    with get_connection() as conn:
        cursor = conn.cursor()
        # Obtener reservas del día actual sin recordatorio enviado
        cursor.execute("""
